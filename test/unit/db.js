...
Lab.experiment('db', function () {
  Lab.experiment('checkDbExists', function () {
    ...
    Lab.test('returns true when db does exist', function (done) {
      Lab.expect(db.checkDbExists('existsTest.sqlite3')).to.equal(true);
      done();
    });
  ...
  Lab.experiment('createEmptyDb', function () {
    ...
    Lab.test('should create the empty db', function (done) {
      db.createEmptyDb('createTest.sqlite3', function (err) {
    ...
    Lab.test('should fail with bad db name', function (done) {
      db.createEmptyDb('schema.sql', function (err) {
        Lab.expect(err.code).to.equal('SQLITE_NOTADB');
        done();
      })
    });
